...
const CarousellPortfolio = ({ showPanel, closePanel }) => {
  ...
  const [buttonAnimationKey, setButtonAnimationKey] = useState(0); // To re-trigger button animation

  const handleAddItems = () => {
    setCount((prevCount) => prevCount + 6); // Add 5 items
    setButtonAnimationKey((prevKey) => prevKey + 1); // Increment to re-trigger button animation
  };
  const images = [
        ...
        <div className='w-full flex flex-col lg:grid lg:grid-cols-3 gap-2 px-2 lg:p-0'>
          {Array.from({ length: count }).map((_, index) => (
            <motion.div
              ...
              transition={{ duration: 0.5, delay: .4 }} // Reset delay every 5 items
          ...
          >
            ...
            View More Project <ArrowLongRightIcon className='w-6' />
